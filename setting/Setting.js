    ...
    loadFieldsTable : function() {
      ...
      array.forEach(this.layersTable.getRows(), function(row) {
        ...
        if (rowData.update === true) {
          ...
          var layer = this.map.getLayer(rowData.id);
          var fields = layer.infoTemplate.info.fieldInfos;//this.getEditableFields(layer.infoTemplate.info.fieldInfos);
          if (firstLay === true) {
            commonFields = fields;
            firstLay = false;
          } else {
      ...
      LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function(operLayerInfos) {
        array.forEach(operLayerInfos._finalLayerInfos,lang.hitch(this,function(layer){
          if (layer.layerObject !== null && layer.layerObject !== undefined) {
            if (layer.layerObject.type === 'Feature Layer' && layer.layerObject.url) {
