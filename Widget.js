  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    loadLayerTable : function() {
      ...
      LayerInfos.getInstance(this.map, this.map.itemInfo).then(lang.hitch(this, function(operLayerInfos) {
        array.forEach(operLayerInfos._finalLayerInfos,lang.hitch(this,function(layer){
          if (layer.layerObject !== null && layer.layerObject !== undefined) {
            if (layer.layerObject.type === 'Feature Layer' && layer.layerObject.url && layer.layerObject.isEditable() === true) {
              ...
              if (layerInfo.layerObject.relationships.length>0 && layerInfo.layerObject.type == "Feature Layer"){
                array.forEach(layerInfo.layerObject.relationships,lang.hitch(this,function(relationship){
                  var signal = _viewerMap.infoWindow.on("show",lang.hitch(this,function(){
                    ...
                    array.forEach(layerInfo.layerObject.getSelectedFeatures(),function(feature){
                      relatedQuery.objectIds.push(feature.attributes[feature._layer.objectIdField]);
                    })
                    ...
                    layerInfo.layerObject.queryRelatedFeatures(relatedQuery,lang.hitch(this,function(relatedRecords){
                      array.forEach(layerInfo.layerObject.getSelectedFeatures(),function(feature){
                        if (Object.keys(relatedRecords).indexOf(feature.attributes[feature._layer.objectIdField].toString()) == -1){
                          feature._isRelated = -1;
                        }else{
                          feature._isRelated = relatedRecords[feature.attributes[feature._layer.objectIdField]].features[0].attributes[relatedRecords[feature.attributes[feature._layer.objectIdField]].features[0]._layer.objectIdField];
                          feature._relatedOidField = relatedRecords[feature.attributes[feature._layer.objectIdField]].features[0]._layer.objectIdField;
                        }
                      })
                      relatedChange = function(fieldName, foreignKeyField) {
                        relatedRecordsUpdate = []
                        relatedRecordsAdd = []
                        array.forEach(layerInfo.layerObject.getSelectedFeatures(),function(feature){
//relatedRecords[layerInfo.layerObject.getSelectedFeatures()[0].attributes[layerInfo.layerObject.getSelectedFeatures()[0]._layer.objectIdField]]
                          var relatedChangeRecord = {
                            attributes: {}
                          };
                          relatedChangeRecord.attributes[foreignKeyField]=feature.attributes[relationship.keyField];
                          dojo.query(".relatedItems").forEach(function(item){
                            if (item.childNodes[1].childNodes[0].childNodes[0].childNodes[0].value != ""){
                              relatedChangeRecord.attributes[dojo.query(item.childNodes[0]).text()] = item.childNodes[1].childNodes[0].childNodes[0].childNodes[0].value
                            }
                          })
                          if (feature._isRelated > -1){
                            relatedChangeRecord.attributes[feature._relatedOidField] = feature._isRelated;
                            relatedRecordsUpdate.push(relatedChangeRecord);
                          }else{
                            relatedRecordsAdd.push(relatedChangeRecord);
                          }
                        })
                        relatedTable.applyEdits(relatedRecordsAdd,relatedRecordsUpdate,null);  
                      };
